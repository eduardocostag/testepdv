...
assistantRouter.post('/ask', async (req,res)=> {
  ...
  if (q.includes('estoque')) {
    ...
    const low = items.filter((i:any)=>Number(i.currentStock)<=Number(i.minStock)).map((i:any)=>i.name).join(', ') || 'nenhum item crítico';
  ...
  if (q.includes('motoboy')) {
    ...
    const best = couriers.sort((a:any,b:any)=>b.orders.length-a.orders.length)[0];
