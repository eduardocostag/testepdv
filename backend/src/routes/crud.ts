...
async function categoryIdFromName(companyId: string, categoryName?: string) {
...
crudRouter.delete('/products/:id', async (req,res)=> {
  ...
  if (linked) return res.status(409).json({ message: 'Produto possui historico de pedidos. Pause o produto em vez de excluir.' });
...
crudRouter.delete('/couriers/:id', async (req,res)=> {
  ...
  if (linked) return res.status(409).json({ message: 'Motoboy possui historico de entregas e nao pode ser excluido.' });
...
crudRouter.delete('/employees/:id', async (req,res)=> {
  if (req.params.id === req.user!.id) return res.status(409).json({ message: 'Voce nao pode excluir seu proprio usuario.' });
...
crudRouter.post('/employees/:id/checkout', async (req,res)=> {
  ...
  if (!open) return res.status(400).json({ message: 'Nenhum ponto aberto' });
