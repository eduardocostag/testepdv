...
async function recalcOrder(orderId: string) {
  ...
  const total = items.reduce((s:number, i:any) => s + Number(i.unitPrice) * i.quantity, 0);
...
ordersRouter.post('/', async (req,res)=> {
  ...
  if (data.tableId) await prisma.restaurantTable.update({ where: { id: data.tableId }, data: { status: 'OCCUPIED' } });
...
ordersRouter.patch('/:id/courier', async (req,res)=> {
  ...
  if (req.body.courierId) await prisma.courier.update({ where: { id: req.body.courierId }, data: { status: 'DELIVERING' } });
  res.json(order);
});
...
ordersRouter.post('/:id/payments', async (req,res)=> {
  ...
  if (paid >= Number(order.total)) await prisma.order.update({ where: { id: order.id }, data: { status: order.origin === 'HALL' ? 'DELIVERED' : order.status } });
