...
ifoodRouter.put('/config', async (req, res) => {
    ...
    if (!current && !clientSecret)
        return res.status(400).json({ message: 'Informe o Client Secret na primeira configuracao.' });
...
ifoodRouter.post('/config/test', async (req, res) => {
    ...
    if (!config.clientId || !config.clientSecret || !config.merchantId)
        return res.status(400).json({ message: 'Preencha Merchant ID, Client ID e Client Secret.' });
    res.json(await testIfoodConnection(config));
});
ifoodRouter.post('/sync', async (req, res) => {
    const config = await prisma.ifoodIntegration.findFirst({ where: { companyId: req.user.companyId } });
    if (!config)
        return res.status(400).json({ message: 'Configure a integracao iFood primeiro.' });
    res.json(await syncIfoodCompany(config));
});
...
ifoodRouter.patch('/external/:externalOrderId/status', async (req, res) => {
    ...
    if (!found)
        return res.status(404).json({ message: 'Pedido externo nao encontrado' });
