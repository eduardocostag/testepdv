...
async function request(config, path, init = {}) {
    ...
    if (!response.ok)
        throw new Error(`iFood ${response.status}: ${await response.text()}`);
...
async function getAccessToken(config) {
    ...
    if (!response.ok)
        throw new Error(`Falha na autenticacao iFood (${response.status}): ${await response.text()}`);
    const data = await response.json();
    const token = data.accessToken || data.access_token;
    if (!token)
        throw new Error('Token iFood nao retornado.');
    tokenCache.set(config.companyId, { token, expiresAt: Date.now() + Number(data.expiresIn || data.expires_in || 21600) * 1000 });
    return token;
}
async function ensureExternalProduct(companyId, item) {
...
async function importOrder(config, externalOrderId) {
    const existing = await prisma.order.findFirst({ where: { companyId: config.companyId, externalOrderId } });
    if (existing)
        return existing;
    const detail = await request(config, `/order/v1.0/orders/${externalOrderId}`);
    ...
    for (const item of detail.items || []) {
        ...
        await prisma.orderItem.create({
            data: {
                ...
                notes: item.observations || item.options?.map((option) => option.name).join(', ') || null
            }
        });
    }
    return order;
}
async function acknowledge(config, eventIds) {
    if (!eventIds.length)
        return;
...
export async function testIfoodConnection(config) {
    await getAccessToken(config);
    return { ok: true, message: 'Autenticacao com iFood realizada com sucesso.' };
}
export async function syncIfoodCompany(config) {
    const events = await request(config, `/events/v1.0/events:polling?merchantId=${encodeURIComponent(config.merchantId)}`) || [];
    const acknowledged = [];
    let imported = 0;
...
export async function syncIfoodStatus(companyId, externalOrderId, status) {
    const config = await prisma.ifoodIntegration.findFirst({ where: { companyId, enabled: true } });
    if (!config)
        return;
    const endpoint = {
        ACCEPTED: 'confirm',
...
export function startIfoodWorker() {
    ...
    const run = async () => {
        ...
        polling = true;
        try {
            const configs = await prisma.ifoodIntegration.findMany({ where: { enabled: true } });
