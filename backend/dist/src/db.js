...
function applyInclude(name, item, include) {
    if (!include)
        return item;
    const result = { ...item };
    if (name === 'order') {
        if (include.table)
            result.table = clone(collections.restaurantTable.find(t => t.id === item.tableId) || null);
        if (include.customer)
            result.customer = clone(collections.customer.find(c => c.id === item.customerId) || null);
        if (include.courier)
            result.courier = clone(collections.courier.find(c => c.id === item.courierId) || null);
        if (include.items) {
        ...
        if (include.payments)
            result.payments = collections.payment.filter(p => p.orderId === item.id).map(p => ({ ...p }));
    }
    if (name === 'restaurantTable') {
        if (include.orders) {
            let orders = collections.order.filter(o => o.tableId === item.id);
            if (include.orders.where)
                orders = orders.filter(o => matchWhere(o, include.orders.where));
    ...
    if (name === 'product') {
        if (include.category)
            result.category = clone(collections.category.find(c => c.id === item.categoryId) || null);
    ...
    if (name === 'courier') {
        if (include.orders)
            result.orders = collections.order.filter(o => o.courierId === item.id).map(o => ({ ...o }));
    }
    if (name === 'user') {
        if (include.timeClocks)
            result.timeClocks = collections.timeClock.filter(t => t.userId === item.id).map(t => ({ ...t }));
    }
    return result;
}
...
function makeCollection(name, initial = []) {
    ...
    return {
        findMany: async ({ where, include, orderBy, take, select } = {}) => {
        ...
        findFirst: async function ({ where, include, orderBy } = {}) {
        ...
        createMany: async ({ data }) => {
            for (const d of data)
                items.push({ id: id(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), ...d });
        ...
        deleteMany: async ({ where } = {}) => {
            ...
            for (let index = items.length - 1; index >= 0; index--) {
                if (matchWhere(items[index], where))
